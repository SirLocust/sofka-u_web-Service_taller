...
public class PersonController {
  ...
  public ResponseEntity<Map<String, Object>> removePerson(@PathVariable() Long id) {
    Person personDb = this.personService.delete(id);
    Map<String, Object> body = new HashMap<>();
    body.put("data", personDb);
    if (personDb == null) {
      body.put("message", "Persona No Eliminada");
      body.put("errors", "Figura no existe");
      return ResponseEntity.status(HttpStatus.NOT_FOUND).body(body);
    }
    body.put("message", "Persona Eliminada con exito");
    body.put("errors", null);
    return ResponseEntity.status(HttpStatus.OK).body(body);
  }
